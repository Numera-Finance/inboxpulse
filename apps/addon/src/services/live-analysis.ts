...
export async function classifyThreadMode(input: {
  ...
  const prompt = [
    ...
    'fyi = needs no action from the recipient: an announcement, a notification,',
    '      or thanks/praise/congratulations. A thread whose whole content is',
    '      appreciation is fyi even when it is warm and long and names people --',
    '      nothing is owed, so there is nothing to do.',
    '',
    `Subject: ${input.subject ?? '(none)'}`,
